...
} from "../../src/lib/python-resolver.js";
...
describe("requireSupportedPython", () => {
  ...
  it("throws when the detected version is below the supported floor", () => {
    ...
    expect(() => requireSupportedPython("python3")).toThrow(
      'Python 3.8.18 detected via "python3", but trellis-tiny init requires Python ≥ 3.9.',
    );
  });
  ...
  it("throws when the command is missing", () => {
    ...
    expect(() => requireSupportedPython("python")).toThrow(
      'Python command "python" not found. trellis-tiny init requires Python ≥ 3.9.',
    );
  });
...
describe("getPythonCommandForPlatform", () => {
  ...
  it("falls back to static platform defaults when nothing is resolved", () => {
    resetResolvedPythonCommand();
