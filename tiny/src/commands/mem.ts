/**
 * mem.ts — CLI wrapper over the vendored `lib/mem` retrieval core.
 *
 * The reusable retrieval / context-extraction logic lives in `lib/mem`
 * (ported from upstream trellis-core); this file owns only CLI concerns:
 * argument parsing, terminal rendering, warning presentation, and process
 * exit behavior.
 *
 * Commands:
 ...
 *
 * Run `tt mem help` for the full flag reference.
 */
...
function cmdHelp(): void {
  console.log(`tt mem — list/search Claude/Codex/Devin/Grok/OpenCode/Pi/ZCode sessions
...
examples:
  tt mem list
  tt mem list --global --platform claude --since 2026-04-01
  tt mem search "session insight" --global
  tt mem extract 5842592d --grep memory
  tt mem extract 5842592d --phase brainstorm
`);
}
