 ...
 * (rank 100) and `<projectRoot>/.agents/skills` (rank 200). Three output
 * paths:
 * - `.agents/skills/` — workflow + bundled skills, written via the NEUTRAL
 *   resolver so the files stay byte-identical to the codex/zcode writes
 ...
 *   configDir-owned tracked file so update/uninstall can detect and scope
 *   dsh.
 *
 ...
 *
 * Ported from upstream `packages/cli/src/configurators/dsh.ts`.
 */
...
function resolveDshCommandSkills(): ResolvedTemplate[] {
...
/**
 * The dsh file set — written at init and diffed by update.
 */
export function collectDshTemplates(): Map<string, string> {
  ...
  //    rendering, byte-identical to codex/zcode writes).
