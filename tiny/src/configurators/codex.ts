...
/**
 * The Codex file set — written at init and diffed by update.
 ...
 *   them (zcode and dsh write the same root in trellis-tiny; last-writer-wins
 *   is only safe when all writers produce identical output).
...
export async function configureCodex(cwd: string): Promise<void> {
  ...
  // user keys are grafted onto exactly the bytes `update` compares
  // against — update runs preserveCodexAgentModelKeys over its already
  ...
  // conventional place for their own.
  ...
  // fallback in inject-workflow-state.py covers this case).
