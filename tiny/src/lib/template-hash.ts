...
/** Path -> SHA256 hash. Keys are POSIX-style repo-relative paths. */
...
/**
 * Read the set of manifest keys from ANY schema version — v1 flat records
 * (`{"path": "hash"}`) and v2 (`{ __version, hashes }`) alike — as an
 * OWNERSHIP CREDENTIAL only.
 *
 * Hash values are deliberately NOT returned: a previous installer (an
 * upstream Trellis manifest, or any legacy writer) may use a different hash
 * algorithm or line-ending semantics, so its hash values must never be
 * compared against or copied into tiny's manifest. Callers re-hash the on-
 * disk content with {@link computeHash} instead.
 */
export function loadKnownTemplatePaths(cwd: string): Set<string> {
  const hashesPath = getHashesPath(cwd);
  if (!fs.existsSync(hashesPath)) {
    return new Set();
  }
  try {
    ...
    if (
      ...
      return new Set(
        Object.keys(
          normalizeHashKeys((parsed as { hashes: TemplateHashes }).hashes),
        ),
      );
    }
    // v1 flat format: the top-level keys are the tracked paths.
    return new Set(Object.keys(parsed).map(toPosix));
  } catch {
    return new Set();
  }
}
 ...
 *
 * The `__pycache__/` / `.pyc` / `.runtime/` entries are load-bearing for the
 * M8 dogfood contract: running the workflow scripts regenerates bytecode
 * caches (often for several Python versions) and the hooks write session /
 * shell-ticket state under `.trellis/.runtime/`. If any of those reached the
 * manifest, the next `update` would classify them as retired templates and
 * DELETE them (session pointers included). Derived/runtime files must never
 * enter the manifest — see {@link isDerivedArtifactPath}.
 */
const EXCLUDE_FROM_HASH = [
  ...
  "__pycache__/", // Python bytecode caches (derived, host-generated)
  ".pyc", // Compiled Python modules
  ".runtime/", // Hook/session runtime state (sessions, shell tickets)
];
...
/**
 * Whether a manifest key / relative path denotes a DERIVED or RUNTIME
 * artifact instead of a shipped template: Python bytecode caches, compiled
 * modules, and hook-written runtime state (`.trellis/.runtime/**`).
 *
 * Command-level guard used by init (manifest carry-over) and update (retire
 * candidates + manifest purge). Keys are POSIX-style; the predicate
 * normalizes anyway so a platform-separator form can never slip through.
 */
export function isDerivedArtifactPath(relativePath: string): boolean {
  const normalized = toPosix(relativePath);
  return (
    normalized.includes("__pycache__/") ||
    normalized.endsWith(".pyc") ||
    normalized === ".trellis/.runtime" ||
    normalized.startsWith(".trellis/.runtime/") ||
    normalized.includes("/.runtime/")
  );
}

/**
 * Drop derived/runtime entries from a hashes record (does not mutate the
 * input). Used to self-heal a manifest poisoned by an older init whose
 * `.trellis/` walk still tracked bytecode caches and runtime state.
 */
export function stripDerivedEntries(hashes: TemplateHashes): TemplateHashes {
  const out: TemplateHashes = {};
  for (const [key, value] of Object.entries(hashes)) {
    if (!isDerivedArtifactPath(key)) out[key] = value;
  }
  return out;
}
...
export interface InitializeHashesOptions {
  /**
   * POSIX-style relative paths trellis-tiny actually wrote during the init
   * run (captured via `startRecordingWrites` in `file-writer.ts`). Only these
   * paths are hashed for the platform/root-level coverage; anything else
   * under `.codex/` / `.zcode/` / etc. is left alone, even if it exists
   ...
   * instead of replacing it. Used by reinit "add platform" flows so
 ...
 * where a blind directory walk of `.codex/` / `.zcode/` swept up
 ...
 * content — accuracy there doesn't affect data-loss, only `update`'s
...
export function initializeHashes(
  ...
  if (trackedPaths) {
    for (const relativePath of trackedPaths) {
      ...
      // Derived/runtime files are never manifest entries (defense in depth —
      // recorded writes should never contain them anyway).
      if (isDerivedArtifactPath(relativePath)) continue;
  ...
  // update's 3-way merge of workflow.md / config.yaml / scripts;
