/**
 * Python command detection and resolution.
 *
 * Extracted from the upstream init flow into a standalone library module so
 * init/update and (future) configurators share one resolver.
 *
 ...
 * before giving up — fixes upstream #236 where users with only `python3`
 * (not `python`) had init fail outright.
 *
...
export function requireSupportedPython(command: string): string {
  ...
  if (!versionOutput) {
    throw new Error(
      `Python command "${command}" not found. trellis-tiny init requires Python ≥ 3.9.`,
    );
  }

  if (!isSupportedPythonVersion(versionOutput)) {
    throw new Error(
      `${versionOutput} detected via "${command}", but trellis-tiny init requires Python ≥ 3.9.`,
    );
  }
...
//
// Which Windows command works varies by installer (python.org / Microsoft
// Store / py launcher). init detects which is available, then calls
// `setResolvedPythonCommand` so all subsequent template / configurator
// writes use the resolved value instead of the platform default.
//
...
export function resolveSupportedPython(): {
  ...
  throw new Error(
    `No supported Python command found. Tried: ${candidates.join(", ")}.\n` +
      `Probe results:\n  ${probeFailures.join("\n  ")}\n\n` +
      `trellis-tiny init requires Python ≥ 3.9. ${installHint}\n` +
