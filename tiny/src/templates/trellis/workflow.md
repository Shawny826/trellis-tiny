...
```
Phase 1: Plan    → classify: simple = zero-task direct path (no task); complex = consent + planning artifacts
...
- Simple task (single package, few files, no architecture impact, immediately verifiable): default to the zero-task direct path — do NOT create a task directory and do NOT ask. Load the relevant specs via `trellis-before-dev`, implement inline, verify with `trellis-check` (lint / type-check / tests), then commit.
- Complex task (multi-deliverable, architecture impact, cross-package): ask whether you may create a Trellis task and enter planning. If the user says no, do not do broad inline implementation; explain, clarify scope, or suggest a smaller split.
- User approval to create a task is not approval to start implementation. Planning still happens first.
- The user can upgrade a direct-path turn to the task workflow in any turn (e.g. "make this a task"); when that happens, start Phase 1.0 from scratch.
...
No active task. Classify the request first: simple work runs on the zero-task fast path by default; only complex work proposes a Trellis task.
Simple task (single package, few files, no architecture impact, immediately verifiable): do NOT create a task directory and do NOT ask — load the relevant specs via `trellis-before-dev`, implement inline, verify with `trellis-check` (lint / type-check / tests), then commit.
Complex / multi-deliverable / architecture-impacting work: ask the user for task-creation consent, then enter Phase 1 planning (consent semantics are preserved for this tier only).
The user can upgrade to the task workflow in any turn (e.g. "make this a task") — when that happens, start Phase 1.0 from scratch.
[/workflow-state:no_task]
...
- No active task defaults to the zero-task direct path; task-creation consent is asked only for complex work (multi-deliverable, architecture impact).
