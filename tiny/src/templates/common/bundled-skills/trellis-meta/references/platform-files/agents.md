...
dsh ships no project-level sub-agent surface — implement/check/research run inline through the workflow skills.

Codex agent profiles support user-set `model` / `model_reasoning_effort` keys; `tt update` preserves them across template refreshes.
