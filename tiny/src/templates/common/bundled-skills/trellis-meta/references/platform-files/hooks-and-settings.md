...
dsh is a pull-based platform with no project-level settings/hooks file trellis-tiny writes — its agent-facing behavior lives in the workflow skills and `.dsh/DSH.md`. ZCode uses `.zcode/config.json` with shared hooks, including PreToolUse for sub-agent prompt injection. Codex additionally requires `features.hooks = true` in the user's global `~/.codex/config.toml` (Codex 0.129+; approve the installed hooks once via `/hooks`).

Whether these files exist in a project depends on which `tt init --<platform>` flags the user ran.
