...
tt mem search "<keyword>"

# Dump dialogue from one session, optionally filtered by phase or keyword.
tt mem extract <session-id> --phase brainstorm
tt mem extract <session-id> --grep "<keyword>"

# Drill into a session: top-N hit turns + surrounding context.
tt mem context <session-id> --turns 3 --around 2

# When you do not know the session id yet, start with list + filter.
tt mem list --cwd <project-path>
tt mem projects   # → list active project cwds, then narrow
```
