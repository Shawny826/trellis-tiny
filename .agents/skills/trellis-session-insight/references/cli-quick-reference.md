# `tt mem` CLI Reference

Full flag reference for the five subcommands. Pin this as the authoritative source — `tt mem help` prints the same content at runtime, so anything here that drifts is a bug.
...
Run `tt mem help` in the user's shell. The runtime help is authoritative and will be ahead of this reference during fast-moving beta releases.
