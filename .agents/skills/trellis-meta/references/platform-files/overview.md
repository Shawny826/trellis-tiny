...
When a local AI modifies trellis-tiny, it should distinguish two file categories first:

- **Shared files**: `.trellis/workflow.md`, `.trellis/tasks/`, `.trellis/spec/`, `.trellis/scripts/`.
- **Platform files**: `.codex/`, `.zcode/`, `.dsh/`, and the shared `.agents/skills/` root.

Platform files do not store business state. They let the corresponding AI tool read trellis-tiny state, call its scripts, and load its skills/agents/hooks.

## Platform File Categories

| Category | Common paths | Purpose |
| --- | --- | --- |
| settings/config | `.zcode/config.json`, `.codex/hooks.json` | Register hooks, plugins, extensions, or platform behavior. |
| hooks/plugins/extensions | `.zcode/hooks/`, `.codex/hooks/` | Inject context at session start, user input, agent startup, shell execution, and similar events. |
| agents | `.codex/agents/`, `.zcode/agents/` | Define `trellis-research`, `trellis-implement`, and `trellis-check`. |
| skills | `.agents/skills/` (the single shared root for codex / zcode / dsh), `.dsh/skills/` (dsh command-entry skills) | Capability descriptions that auto-trigger or can be read on demand. |
| commands/prompts/workflows | `.zcode/commands/trellis/` | Entry points explicitly invoked by the user. |
