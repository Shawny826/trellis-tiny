...
`trellis-meta` is for user projects that have already run `tt init`. The user's machine usually has only the npm-installed `tt` command plus the trellis-tiny files generated inside the project; it may not have the trellis-tiny source code.
...
- Platform directories: `.codex/`, `.zcode/`, `.dsh/`.
- Shared skill layer: `.agents/skills/` (the single skill root all three platforms read).
