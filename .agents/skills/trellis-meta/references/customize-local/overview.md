...
This directory is for local AI working in a user project where trellis-tiny was installed through npm and `tt init` has already been run. The AI should modify generated `.trellis/` and platform directories inside the project, not trellis-tiny upstream source code.
...
| Platform integration | `.codex/`, `.zcode/`, `.dsh/`, and the shared `.agents/skills/` root |
| Shared skill | `.agents/skills/` |

## Things Not To Do By Default

- Do not edit the global npm install directory.
- Do not edit `node_modules/trellis-tiny`.
- Do not assume the user has the trellis-tiny repository.
...
- "I want to open a PR to trellis-tiny"
- "I want to change npm package publish contents"
- "I want to fork trellis-tiny"
- "I want to modify the generation logic for `tt init` / `tt update`"
