...
- **Active task status `in_progress`** → Phase 2 step 2.1. Load the step detail:
  ...
  ```
- **No active task** → classify first. Simple task (single package, few files, no architecture impact, immediately verifiable): default to the zero-task direct path — do NOT create a task directory and do NOT ask; load `trellis-before-dev`, implement inline, verify with `trellis-check`, commit. Complex / multi-deliverable / architecture-impacting work: ask whether you may create a Trellis task and enter planning. The user can upgrade a direct-path turn to a task in any turn.
